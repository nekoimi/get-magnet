...
- 资源生命周期操作：状态标记、下载提交/重试、STRM 单条与批量重建、`magnet_events` 事件记录。
- 管理首页统计：`GET /api/v1/dashboard/summary`。
- 下载队列与调度：队列筛选、手动运行、调度配置和最近任务执行结果。
- 运维能力：运行配置只读与脱敏、依赖连接测试、聚合健康检查、版本和运行时信息。
...
- 前端路由中已加入 `/magnets` 菜单。
- 下载队列、资源详情、网盘任务查询、快速提交页面已接入。
- 系统设置、依赖连接测试、健康与版本、调度任务页面已接入。

### 当前剩余问题

- `/quick-api` 仍免鉴权，公网部署时需要 token 或网络隔离。
- 采集 worker、队列、当前任务和 provider 状态尚未形成完整管理页面。
- 播放与 STRM 目前依附于资源详情，缺少集中管理页面。
- 安全日志 tail API 尚未实现，当前仍需通过容器或主机查看日志。
- 模板中仍保留较多系统、示例和功能演示路由。
- 全量测试存在外网依赖、永久阻塞和固定端口冲突问题。
- P3 功能已完成代码实现，但仍需在真实部署环境完成连接和健康检查验收。
...
当前产品定位为单管理员自用系统，不引入复杂 RBAC。多人协作、角色权限、管理员管理和审计能力已明确暂缓，不纳入当前版本路线；只有在实际出现多人使用需求后再重新评估。
...
- 资源生命周期关键操作写入 `magnet_events`；通用操作审计随多人协作能力一并暂缓。
...
### P4：多人协作与审计（暂缓，不纳入当前阶段）

决策说明：

- 当前系统继续采用单管理员模型。
- 本阶段不扩展管理员 CRUD、角色权限、通用操作审计和登录日志。
- 以下条目仅作为未来需求备忘，不参与当前版本验收和完成度统计。
- 重新启动条件：出现两个及以上独立账号、只读访问、操作归责或数据隔离的明确需求。
...
P4 暂缓后，下一阶段聚焦现有业务闭环的稳定性、安全性和可部署性，不再扩展用户体系。

#### 功能补齐

- [x] 实现采集运行状态接口，展示 worker 数、队列长度和当前任务。
- [x] 实现采集 provider 列表与启用状态只读展示；在线修改继续暂缓。
- [x] 增加独立的播放与 STRM 列表页，集中查看已完成资源。
- [x] 增加 `sourceOptions` 接口并由前端统一使用来源枚举。
- [x] 补齐“是否已生成 STRM”筛选条件。
- [x] 评估日志 tail API 的安全边界；当前单管理员版本保持容器日志方案，不开放文件读取接口。

#### 稳定性与测试

- [x] 修复或隔离依赖外网的 tracker 测试，避免全量测试不稳定。
- [x] 修复 `internal/pkg/queue` 中永久阻塞的 `TestQueue`。
- [x] 调整集成测试端口分配，避免固定端口冲突。
- [ ] 为下载队列状态筛选和资源组合筛选增加仓储层测试。
- [ ] 为 settings、ops、下载提交和状态转换增加 handler 或服务层测试。
- [ ] 为数据库迁移增加重复执行和升级路径验证。

#### 安全与上线

- [x] 为 `/quick-api` 增加可选 token 或明确仅允许内网访问。
- [x] 检查日志、配置输出和错误响应，确保不会泄露 JWT、数据库密码、aria2 secret 等敏感信息。
- [ ] 验证反向代理 HTTPS、请求体大小、超时和限流配置。
- [ ] 在实际 cloud-driver 部署清单中补充 `/health` healthcheck。
- [ ] 使用真实部署环境完成 P3 验收项并记录结果。
- [ ] 增加数据库备份、恢复和迁移失败回滚说明。

#### 前端收尾

- [x] 移除或隐藏模板演示页面和无业务意义路由。
- [ ] 统一分页参数、业务实体类型、时间格式、错误提示和空状态展示。
- [ ] 为批量提交、批量重试和状态修改补充二次确认及影响数量。
- [ ] 完成桌面端常用分辨率和移动端基础可用性检查。

#### 文档与发布

- [x] 更新 README，补充在线管理系统能力、配置、构建和部署说明。
- [ ] 校准本文档前半部分“当前能力盘点”和 API 清单，使其与已发布代码一致。
- [ ] 定义版本号和构建时 Git commit 注入方式。
- [ ] 形成上线检查清单和最小回归测试清单。

### 技术债清理

- [x] 移除或隐藏模板演示页面和无业务意义路由。
- [x] 统一前端接口命名风格，避免 `pageNum` 与 `page_num` 混用。
- [x] 为前端业务实体补充 TypeScript 类型定义。
- [x] 统一时间格式展示。
- [x] 统一错误提示和空状态展示。
- [x] 后端补充 API 单元测试或 handler 级测试。
- [x] 后端补充下载状态转换测试。
- [x] 后端补充配置解析和默认值测试。
- [x] 更新 README 的在线管理系统说明。
- [x] 更新 Docker Compose 示例。
