...
import (
	...
	"github.com/nekoimi/get-magnet/internal/model/common"
	"github.com/nekoimi/get-magnet/internal/model/common/model"
	"log"
	"strings"
)

// ChineseSubtitlesMovieList movie list parser
func ChineseSubtitlesMovieList(meta *common.Meta, selection *goquery.Selection) (*common.Out, error) {
	...
	if len(existsPathSet) == 0 {
		...
		if existsNext {
			...
			keepTasks = append(keepTasks, common.NewTask(meta.Host+nextHref, ChineseSubtitlesMovieList))
		}
	}

	for _, href := range notExistsPathArr {
		keepTasks = append(keepTasks, common.NewTask(meta.Host+href, MovieDetails))
	}
...
func MovieDetails(meta *common.Meta, s *goquery.Selection) (*common.Out, error) {
